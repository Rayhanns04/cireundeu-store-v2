...
@section('title', 'Cireundeu Store | Produk')
...
@section('modal')
    <div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form enctype="multipart/form-data" action="/product-import" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="recipient-name" class="col-form-label">File product excel</label>
        ...
        <div class="modal fade" id="createModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form enctype="multipart/form-data" action="/products/save-create" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="recipient-name" class="col-form-label">Image URL (174px X 156px)</label>
        ...
        @foreach ($products as $product)
            <div class="modal fade" id="editModal{{ $product->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <form action="{{ $Action }}/save-edit/{{ $product->id }}" method="POST"
                                ...
                                <div class="mb-3">
                                    <img src="{{ $product->image }}" class="img-responsive img-rounded"
                                        style="max-height: 500px; max-width: 200px;">
                                </div>
                                ...
                                {{-- <div class="mb-3">
                                    <label class="form-label">Image (174px X 156px)</label>
                                    <input class="form-control" type="file" id="image" name="image"
                                        value="{{ asset('assets/images/productsImage/' . $product->image) }}">
                                </div> --}}
                                ...
                                <div class="mb-3">
                                    <label for="recipient-name" class="col-form-label">Dekskripsi</label>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" aria-label="Default select example" name="category_name">
                                        ...
                                        <option selected>Open this select menu ( {{ $product->subCategory->name }} )
                                        </option>
                                    ...
                                    <td class="text-bold-500 mw-10">
                                        <div class="form-check form-check-inline">
                                            ...
                                            <label class="form-check-label text-bold-500" for="inlineRadio1">
                                                {{ $products->firstItem() + $key }}</label>
                                        </div>
                                    </td>
                                    <td><a href="{{ $item->image }}" target="_blank">Lihat gambar</a>
                                    </td>
                                    <td>{{ $item->title }}</td>
                                    <td>
                                        <div style="max-height:50px; overflow:auto; display: inline-block">
                                            {{ $item->description }}</div>
                                    </td>
                                    <td>Rp {{ number_format($item->price) }}</td>
                                    <td><span
                                            class="badge rounded-pill bg-primary text-light font-size-14 px-3 py-2">{{ $item->subCategory->name }}</span>
                                    </td>
                                    <td>
